...
#pdf_g = 10*stats.norm.pdf(x,mu,sigma) # now get theoretical values in our interval  
#plt.plot(x, pdf_g, label="Normal") # plot it

# gamma dist - best
ag,bg,cg = gamma.fit(capacity)  
pdf_gamma = 10*gamma.pdf(x,ag,bg,cg)  
plt.plot(x, pdf_gamma, label="Gamma")

## beta dist
#ab,bb,cb,db = stats.beta.fit(capacity)  
#pdf_beta = 10*stats.beta.pdf(x,ab,bb,cb,db)  
